view: android_focus_test_results {
  sql_table_name: `moz-mobile-tools.testops_results.focus_ui_tests` ;;
  ...
  # Health status based on rates with recency awareness
  dimension: test_health_status {
    type: string
    sql: CASE
      WHEN ${total_runs} < 10 THEN 'Low Sample'
      WHEN SAFE_DIVIDE(${TABLE}.failed_runs, ${TABLE}.total_runs) > 0.01
        AND ${TABLE}.last_failure_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY)
        THEN 'Failing'
      WHEN SAFE_DIVIDE(${TABLE}.failed_runs, ${TABLE}.total_runs) > 0.01
        THEN 'Recovered'
      WHEN SAFE_DIVIDE(${TABLE}.flaky_runs, ${TABLE}.total_runs) > 0.01
  ...
  # Test package (e.g., org.mozilla.focus.ui)
