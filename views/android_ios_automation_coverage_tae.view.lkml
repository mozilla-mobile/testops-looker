view: android_ios_automation_coverage_tae {
  derived_table: {
    sql:
        ...
        SELECT
          ...
          END AS category,
          SUM(CASE WHEN status = 'Completed' AND test_automation_tae = 1 THEN test_count_sum ELSE 0 END) AS automated_tae,
          SUM(CASE WHEN status = 'Completed' AND test_automation_tae = 0 THEN test_count_sum ELSE 0 END) AS automated_no_tae,
  ...
  dimension: automated_tae {
    type: number
    sql: ${TABLE}.automated_tae ;;
  }

  dimension: automated_no_tae {
    type: number
    sql: ${TABLE}.automated_no_tae ;;
  }
  ...
  dimension: tae_pct {
    type: number
    sql: ROUND(SAFE_DIVIDE(${TABLE}.automated_tae, ${TABLE}.automated) * 100, 1) ;;
    value_format: "0.0\%"
    description: "% of automated tests carrying the TAE label"
  }

  measure: total_automated_tae {
    type: sum
    sql: ${automated_tae} ;;
    label: "Automated (TAE)"
  }

  measure: total_automated_no_tae {
    type: sum
    sql: ${automated_no_tae} ;;
    label: "Automated (no TAE)"
  }
